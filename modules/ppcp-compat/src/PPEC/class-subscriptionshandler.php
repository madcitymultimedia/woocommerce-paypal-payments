...
class SubscriptionsHandler {
	 ...
	 * @return bool
	 */
	public function site_has_ppec_subscriptions() {
		global $wpdb;
	...
	public function maybe_hook() {
		if ( ! $this->site_has_ppec_subscriptions() ) {
			return;
		}
		...
		add_action( 'woocommerce_scheduled_subscription_payment_' . self::PPEC_GATEWAY_ID, array( $this, 'process_renewal' ), 10, 2 );

		// Frontend.
		add_filter( 'wcs_view_subscription_actions', array( $this, 'disable_subscription_actions' ), 999, 2 );
	}
	...
	public function add_mock_ppec_gateway( $gateways ) {
		if ( ! isset( $gateways[ self::PPEC_GATEWAY_ID ] ) && $this->should_mock_ppec_gateway() ) {
			$gateways[ self::PPEC_GATEWAY_ID ] = $this->mock_gateway;
		}
	...
	public function use_billing_agreement_as_token( $token, $customer, $order ) {
		if ( self::PPEC_GATEWAY_ID === $order->get_payment_method() && wcs_order_contains_renewal( $order ) ) {
	...
	/**
	 * Limits subscription actions on the frontend when using the mock gateway.
	 *
	 * @param array            $actions      Current subscription actions.
	 * @param \WC_Subscription $subscription Subscription object.
	 * @return array
	 */
	public function disable_subscription_actions( $actions, $subscription ) {
		return $actions;
	}
	...
	private function should_mock_ppec_gateway() {
		...
		if ( wcs_is_view_subscription_page() ) {
			...
			return ( $subscription && self::PPEC_GATEWAY_ID === $subscription->get_payment_method() );
		}
		...
		if ( function_exists( 'get_current_screen' ) && get_current_screen() ) {
			...
			if ( in_array( get_current_screen()->id, array( 'shop_subscription', 'shop_order' ), true ) ) {
				...
				return ( $order && self::PPEC_GATEWAY_ID === $order->get_payment_method() );
			}
		}
